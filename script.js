...
const UPDATE_HISTORY = [
  ...
  { version: 'v1.21.0', notes: [
    ...
    '잘림 없는 전체 보기, 화면 채우기, 전체화면과 메뉴 숨기기를 지원해요'
  ]},
...
const DEVELOPER_NOTES = [
  ...
  },
  {
    ...
    body: '특별실로 이동하기 전이나 토의활동 전에, 미리 만든 규칙 이미지를 필요할 때 화면 가득 띄우고 싶다는 건의를 보내주셨어요. 수업에서 유용하게 사용하고 계시다는 말씀과 함께 구체적인 활용 장면을 알려주셔서 감사합니다! 이번 업데이트에 "이미지 띄우기" 기능을 준비했습니다.\n\n화면 하단의 학급 도구 > 이미지 띄우기, 또는 학급 도구 버튼 위의 작은 그림 버튼을 눌러주세요. "+ 이미지 추가"로 규칙이나 안내 사진을 여러 장 등록하고, 알아보기 쉬운 이름을 붙일 수 있어요. 보관함에서 원하는 이미지를 누르면 크게 표시됩니다.\n\n기본은 글자나 그림이 잘리지 않도록 이미지 전체를 보여주는 방식이에요. 화면 비율에 따라 여백이 생길 수 있으며, "화면 채우기"를 선택하면 일부가 잘리는 대신 화면을 가득 채웁니다. "전체화면"과 "메뉴 숨기기"를 함께 사용하면 이미지에 집중해서 볼 수 있고, 닫기 버튼이나 Esc로 보관함에 돌아올 수 있어요. 이미지는 현재 기기의 같은 브라우저에 저장되어 다음에도 사용할 수 있습니다. 다만 브라우저 데이터를 지우면 삭제되고 설정 내보내기에는 포함되지 않으니 원본 파일은 보관해주세요.\n\n건의해주신 선생님, 제가 요청하신 모습을 잘 이해했는지 확인하고 싶어요. 이번처럼 사진을 미리 저장해두고 필요할 때 골라 크게 띄우는 방식이 원하셨던 기능과 맞을까요? 혹시 시계와 시간표가 보이는 상태에서 배경 사진만 바꾸는 모습이나, 그림 버튼을 누르면 지정한 사진 한 장이 곧바로 뜨는 모습을 생각하셨을 수도 있을 것 같아요. 직접 사용해보시고 화면 하단의 "의견 보내기"로 원하셨던 방식과 다른 점이나 더 편했으면 하는 점을 알려주세요. 수업 중 어떤 순서로 사용하고 싶으신지도 함께 적어주시면 다음 개선에 도움이 되겠습니다!'
  },
  {
  ...
  },
  {
    ...
    date: '2026-09-06',
    title: '시계의 콜론(:) 깜빡임을 끌 수 있어요',
    body: '화면을 계속 띄워두는데 시계의 콜론(:)이 깜빡여서 신경 쓰인다는 문의를 주셨어요. 콜론 깜빡임은 설정에서 언제든 켜고 끌 수 있습니다.\n\n화면 하단의 톱니바퀴(설정)를 누른 뒤, 표시 설정에서 "콜론 깜빡임" 스위치를 꺼주세요.\n\n끄면 시와 분 사이의 콜론(:)이 깜빡이지 않고 계속 표시됩니다. 시간은 그대로 정상적으로 흐르며, 다시 깜빡이게 하고 싶을 때는 같은 스위치를 켜주시면 됩니다.\n\n설정은 별도 저장 버튼 없이 바로 저장되며, 같은 기기와 브라우저에서는 새로고침하거나 사이트를 다시 열어도 유지됩니다.\n\n오래 띄워두는 교실 화면인 만큼 편안하게 보실 수 있도록, 사용 중 불편한 점이 있으면 언제든 알려주세요. 문의해주셔서 감사합니다!'
  },
  {
  ...
  },
  {
    ...
    date: '2026-08-20',
    title: '시작 화면 선택과 중요 메모 기능을 추가했어요',
    body: '사이트를 열 때 자주 사용하는 화면부터 바로 보고 싶다는 필요와, 활동 메모 안에서 중요한 일정이나 내용을 따로 강조하고 싶다는 의견을 반영했습니다.\n\n먼저 설정 > 표시 설정에 "사이트 시작 탭"이 생겼습니다. 기존처럼 마지막으로 본 탭을 이어서 보거나, 학급 약속·알림장·공지사항·급식·디데이 중 하나를 선택해 사이트를 열 때마다 그 탭에서 시작할 수 있어요.\n\n또 학급 약속의 아침 활동, 쉬는 시간, 점심 활동 메모에서 "비우기" 옆의 "+ 중요 메모"를 누르면 기존 메모 하단에 강조된 별도 작성 영역이 열립니다. 10시 소방훈련, 단원평가, 고요한 책 읽기처럼 아이들과 먼저 확인할 내용을 눈에 띄게 나누어 적을 수 있고, 활동별로 각각 저장됩니다. 필요 없을 때는 "영역 삭제"로 정리할 수 있으며 내용이 있으면 삭제 전에 한 번 더 확인합니다.\n\n기존에 작성해둔 활동 메모와 설정은 그대로 유지됩니다. 실제 아침 교실에서 바로 활용할 수 있는 좋은 의견을 보내주셔서 감사합니다!'
  },
  {
